#!/usr/bin/env python3
"""Full Chinese-section GitHub-snapshot-only 2013 corpus builder."""
...
EXCLUDE_DIRS = {"english"}
EXCLUDE_TITLE = re.compile(
    r"(?:速查|Cheat Sheet|下载地址|壁纸|桌面|插件列表|资源汇总|招聘启事|网站推荐|"
    r"纯代码|源码下载|命令列表|照片集|图片集)"
)


def collect_all_chinese_ruanyf(pool: list[dict], repo: Path) -> None:
    for path in sorted((repo / "ruanyifeng").rglob("2013*.rst")):
        relative_parts = set(path.relative_to(repo / "ruanyifeng").parts[:-1])
        ...
        if section in {"computer", "developer", "algorithm", "javascript", "internet", "programmer"}:
            genre = "技术评论 / 方法评述 / 互联网分析"
        elif re.search(r"书|序言|读后感|纪录片|电影", title):
            genre = "书评 / 影评 / 文化评论"
        elif section in {"history", "humanities", "literature", "clipboard"}:
            genre = "历史 / 人文 / 社会评论"
        else:
            genre = "观点 / 社会与科技评论"
        base.add(
            ...
            text=body,
            notes=(
                f"Fixed GitHub mirror commit; source section={section}; official_url={official}; original site states "
                "自由转载-非商用-非衍生-保持署名; title/byline metadata, code/literal blocks, links, "
                "long quotations, images and back matter removed."
            ),
        )
    print("RUANYF_ALL_CHINESE", len([x for x in pool if x["source_platform"] == "阮一峰文章固定仓库快照"]))
...
def main() -> None:
    ...
    pool: list[dict] = []
    collect_all_chinese_ruanyf(pool, ruanyf)
